...
jQuery(document).ready(function($) {
  $(".form-group label").each(function(){
        if($(this).text() === "Languages:"){
           var box =$(this).next().find("label input").eq(0).parents(".checkbox-inline");
			
		   box.find(".iCheck-helper").remove();	
           box.on("click",function(){ 
                
			 if($(this).children().hasClass("checked")){
				 box.siblings(".checkbox-inline").each(function(){
			...
			$(window).load(function(){	
				box.find(".iCheck-helper,input").unbind("click");	
			})
